    ...
    if model_type == 'classic':
        ...
        for lr_index, sublr in enumerate(lr_stack):
            for gm_index, classic_method in enumerate(classic_methods):
                ...
                for iteration in range(num_training_iterations):
                    for sub_iter in range(Tot_iters):
                        ...
                        if ((iteration * Tot_iters + sub_iter) % print_every == 0):
                            print('Iteration:{},Training Loss:{:.3g}'.format(iteration * Tot_iters + sub_iter, loss.item()))
                ...
                hp = hamiltonian_fn(train_pred_state.detach().numpy(), model_type)
                hp_gt = hamiltonian_fn(xnext.detach().numpy(), model_type)
                ...
                hp = hamiltonian_fn(valid_pred_state.detach().numpy(), model_type)
                hp_gt = hamiltonian_fn(test_xnext.detach().numpy(), model_type)
                ...
                for t_iters in range(n_test_traj):
                    ...
                    df_all.loc[len(df_all)] = [classic_method, model_type, t_iters,
                                               train_loss.detach().numpy(), train_std.detach().numpy(), train_energy_error, train_energy_std,
                                               valid_loss.detach().numpy(), valid_std.detach().numpy(), valid_energy_error, valid_energy_std,
