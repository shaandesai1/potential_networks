...
class nongraph_model(torch.nn.Module):
    ...
    def deriv_fun_hnn(self, xt):
        with torch.enable_grad():
            ...
            # print(solenoidal_field.shape)
            return solenoidal_field  # torch.cat([solenoidal_field[0,1].view(1,1),-solenoidal_field[0,0].view(1,1)],1)

    def deriv_fun_pnn(self, xt):
        vdim = int(xt.shape[1] / 2)
        q, p = xt[:, :vdim], xt[:, vdim:]
        output_nodes = self.forward(q)
    ...
    def train_step(self, input_batch, true_batch, steps):
        trajectories = torch.empty((steps + 1, 2), requires_grad=False)

        # print(input_batch.shape)
        yhato = input_batch.reshape(1, -1)
        yhato.requires_grad_()
        trajectories[0, :] = yhato

        for i in range(steps):
            yhatn = self.integ(self.deriv_function, yhato, self.dt)
            trajectories[i + 1, :] = yhatn
            yhato = yhatn.requires_grad_()

        error = ((trajectories[1:] - true_batch) ** 2).mean()
        # print(error)
        return error, trajectories[1:, :]
